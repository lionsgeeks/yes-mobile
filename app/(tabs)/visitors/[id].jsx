  ...
  return (
    <ScrollView className="flex-1 pt-10 bg-[#f9f9f9]">
      ...
      <View className="items-center px-4 mt-8">
        ...
        />
        <Text className="text-2xl font-bold text-[#2e539d]">
          {visitor.name}
        </Text>
      ...
      <View className="px-4 py-6 space-y-4">
        ...
        <View className="bg-white p-4 rounded-xl shadow-sm">
          <Text className="text-lg font-semibold text-[#2e539d] mb-2">
            About
          </Text>
          <Text className="text-sm text-gray-700 leading-relaxed">
            {visitor.description ||
              "This visitor is a leading expert in their field, contributing to impactful initiatives and programs across the globe."}
          </Text>
        </View>
          ...
          <View className="bg-white p-4 rounded-xl shadow-sm">
            ...
            <View className="flex-row flex-wrap gap-2">
              {visitor.languages.map((lang) => (
                <View
                  key={lang}
                  className="bg-[#b09417] px-3 py-1 rounded-full"
                >
          ...
          <View className="bg-white p-4 rounded-xl shadow-sm">
            ...
            {visitor.sessions.map((session, index) => (
              <View
                ...
                style={{ elevation: 1 }}
              >
                <Text className="text-[#2e539d] font-semibold">
                  {session.title}
                </Text>
